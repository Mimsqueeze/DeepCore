...
#define NUM_TEST_BATCHES (NUM_TEST_IMAGES/BATCH_SIZE)
...
streamoff read(float *gpu_X, int rows, int cols, streamoff position, const string &path) {
    // Open file
...
int main() {
    ...
    write_position = read(gpu_W1, L1_SIZE, INPUT_SIZE, write_position, path);
    write_position = read(gpu_B1, L1_SIZE, 1, write_position, path);
    write_position = read(gpu_W2, OUTPUT_SIZE, L1_SIZE, write_position, path);
    read(gpu_B2, OUTPUT_SIZE, 1, write_position, path);
    ...
    for (int i = 0; i < NUM_TEST_IMAGES; i += BATCH_SIZE) {
        ...
        if (PRINT_BATCH_AND_PREDICTIONS) {
            print_batch_and_predictions(gpu_test_X, gpu_test_Y, gpu_test_A2);
        }
        ...
        // Update console
        cout << "\r";
        cout << "BATCH " << (i / BATCH_SIZE) + 1 << "/" << NUM_TEST_BATCHES << " ";
        cout << "[";
